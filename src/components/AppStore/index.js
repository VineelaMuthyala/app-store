...
const initialList = appsList.filter(eachItem => eachItem.category === 'SOCIAL')
console.log(initialList)

class AppStore extends Component {
  state = {selectedList: initialList, searchInputValue: ''}

  tabSelected = tabId => {
    const filteredList = appsList.filter(
      eachItem => eachItem.category === tabId,
    )
    this.setState({selectedList: filteredList})
  }
  ...
  filteredAppsList = () => {
    const {selectedList, searchInputValue} = this.state
    const filteredApps = selectedList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInputValue.toLowerCase()),
    )

    if (filteredApps.length === 0) {
      return <ul className="apps-container"> </ul>
    }
    return (
      <div>
          ...
          <ul className="tab-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isSelected={eachTab.tabId === selectedList[0].category}
              />
            ))}
          </ul>
        </div>
        <div>{this.filteredAppsList()}</div>
      </div>
    )
  }
}
